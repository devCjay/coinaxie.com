    ...
    <div class="space-y-8">
        ...
        <div class="relative overflow-hidden rounded-2xl bg-secondary border border-white/5 p-6 md:p-8">
            ...
            <div class="relative z-1 flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div class="flex items-start gap-5">
                    ...
                    <div>
                        <h1 class="text-2xl md:text-3xl font-bold text-white tracking-tight">{{ __('Withdrawal History') }}
                        </h1>
                        <p class="text-text-secondary text-sm md:text-base mt-2 max-w-xl leading-relaxed">
                            {{ __('Securely manage and track your withdrawals. View detailed transaction history and stay updated on your payout status.') }}
                        </p>
                    </div>
                </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <a href="{{ route('user.withdrawals.index') }}"
                class="bg-secondary border border-white/5 rounded-2xl p-6 relative overflow-hidden group hover:border-accent-primary/20 transition-all">
                ...
                <p class="text-text-secondary text-sm font-medium">{{ __('Total Withdrawn') }}</p>
                <div class="mt-2 flex items-baseline gap-2">
                    <h3 class="text-2xl font-bold text-white">
                        {{ number_format($withdrawals_analytics['total']['total'], 2) }}
                ...
                <div class="mt-4 flex items-center gap-2 text-xs text-text-secondary">
                    <span class="px-2 py-1 rounded bg-white/5 border border-white/5">
                        {{ $withdrawals_analytics['total']['count'] }} {{ __('Transactions') }}
                    </span>
                </div>
            </a>
            ...
            <a href="{{ route('user.withdrawals.pending') }}"
                class="bg-secondary border border-white/5 rounded-2xl p-6 relative overflow-hidden group hover:border-yellow-500/20 transition-all">
                <div class="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                ...
                <div class="mt-2 flex items-baseline gap-2">
                    <h3 class="text-2xl font-bold text-white">
                        {{ number_format($withdrawals_analytics['pending']['total'], 2) }}
                        <span class="text-sm font-normal text-text-secondary">{{ getSetting('currency') }}</span>
                    </h3>
                </div>
                <div class="mt-4 flex items-center gap-2 text-xs text-text-secondary">
                    <span class="px-2 py-1 rounded bg-yellow-500/10 border border-yellow-500/10 text-yellow-500">
                        {{ $withdrawals_analytics['pending']['count'] }} {{ __('Pending') }}
                    </span>
                </div>
            </a>
            ...
            <a href="{{ route('user.withdrawals.approved') }}"
                class="bg-secondary border border-white/5 rounded-2xl p-6 relative overflow-hidden group hover:border-emerald-500/20 transition-all">
                <div class="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                ...
                <div class="mt-2 flex items-baseline gap-2">
                    <h3 class="text-2xl font-bold text-white">
                        {{ number_format($withdrawals_analytics['completed']['total'], 2) }}
                        <span class="text-sm font-normal text-text-secondary">{{ getSetting('currency') }}</span>
                    </h3>
                </div>
                <div class="mt-4 flex items-center gap-2 text-xs text-text-secondary">
                    <span class="px-2 py-1 rounded bg-emerald-500/10 border border-emerald-500/10 text-emerald-500">
                        {{ $withdrawals_analytics['completed']['count'] }} {{ __('Completed') }}
                    </span>
            ...
            {{-- Failed/Partial --}}
            <a href="{{ route('user.withdrawals.failed') }}"
                class="bg-secondary border border-white/5 rounded-2xl p-6 relative overflow-hidden group hover:border-red-500/20 transition-all">
                <div class="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                ...
                <div class="mt-2 flex items-baseline gap-2">
                    <h3 class="text-2xl font-bold text-white">
                        {{ number_format($withdrawals_analytics['failed']['total'] + $withdrawals_analytics['partial_payment']['total'], 2) }}
                        <span class="text-sm font-normal text-text-secondary">{{ getSetting('currency') }}</span>
                    </h3>
                </div>
                <div class="mt-4 flex items-center gap-2 text-xs text-text-secondary">
                    <span class="px-2 py-1 rounded bg-red-500/10 border border-red-500/10 text-red-500">
                        {{ $withdrawals_analytics['failed']['count'] }} {{ __('Failed') }}
                    </span>
                    <span class="px-2 py-1 rounded bg-orange-500/10 border border-orange-500/10 text-orange-500">
                        {{ $withdrawals_analytics['partial_payment']['count'] }} {{ __('Partial') }}
                    </span>
                </div>
            </a>
        </div>
                ...
                <div class="relative z-10">
                    ...
                    <div class="space-y-4">
                        ...
                        <div
                            ...
                            <div class="text-right">
                                <span class="block text-white font-bold">
                                    @if ($slowest_withdrawal)
                                        @php $s_seconds = $slowest_withdrawal->created_at->diffInSeconds($slowest_withdrawal->updated_at); @endphp
                            ...
                            @if ($largest_withdrawal)
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center gap-2">
                                        ...
                                        <span class="text-sm text-text-secondary">{{ __('Largest Withdrawal') }}</span>
                                    </div>
                                    <span
                                        class="text-white font-bold">{{ number_format($largest_withdrawal->amount, 2) }}</span>
                                </div>
                            @endif
                                ...
                                <div class="method-item {{ $loop->index > 2 ? 'hidden' : '' }}">
                                    <div class="flex justify-between text-xs mb-1">
                                        <span class="text-white">
                                            {{ __($stat->withdrawalMethod->name) ?? __('Unknown') }}
                                        </span>
                                    ...
                                    <div class="w-full bg-white/5 rounded-full h-1.5 backdrop-blur-sm">
                                        @php $percent = $withdrawals_analytics['total']['total'] > 0 ? ($stat->total / $withdrawals_analytics['total']['total']) * 100 : 0; @endphp
        ...
        <div id="withdrawals-history-wrapper"
            ...
            <div
                ...
                <h3 class="text-lg font-bold text-white">{{ __('Recent Withdrawals') }}</h3>

                <form method="GET" action="{{ route('user.withdrawals.index') }}" class="relative">
                    <input type="text" name="search" placeholder="{{ __('Search withdrawals...') }}"
                        ...
                        <tbody class="divide-y divide-white/5">
                            @forelse($withdrawals as $withdrawal)
                                <tr class="hover:bg-white/5 transition-colors group">
                                        ...
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                ...
                                                <p class="text-text-secondary text-xs"
                                                    ...
                                                    {{ $withdrawal->transaction_hash ? (strlen($withdrawal->transaction_hash) > 6 ? substr($withdrawal->transaction_hash, 0, 3) . '...' . substr($withdrawal->transaction_hash, -3) : $withdrawal->transaction_hash) : '---' }}
                                                </p>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">
                                        <p class="text-white font-bold">{{ number_format($withdrawal->amount, 2) }} <span
                                    ...
                                    <td class="px-6 py-4">
                                        <p class="text-white text-sm">{{ $withdrawal->created_at->format('M d, Y') }}</p>
                                        <p class="text-text-secondary text-xs">
                                        ...
                                        <div class="flex flex-col items-center justify-center">
                                            @if (request('search'))
                                                ...
                                                <p class="text-text-secondary text-sm mb-6">
                                                    {{ __('We couldn\'t find any withdrawals matching ":search"', ['search' => request('search')]) }}
                                                </p>
                                                <a href="{{ route('user.withdrawals.index') }}"
                                            ...
                                            @else
                                                ...
                                                <p class="text-white font-medium text-lg">{{ __('No withdrawals found') }}
                                                </p>
                                                <p class="text-text-secondary text-sm mb-6">
                                                    {{ __('You haven\'t made any withdrawals yet.') }}</p>
                                                <a href="{{ route('user.withdrawals.new') }}"
                                                    class="text-accent-primary hover:text-accent-primary-hover font-medium text-sm transition-colors">
                                                    {{ __('Make your first withdrawal') }} &rarr;
                                                </a>
                                            @endif
                                        </div>
                                    </td>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (urlParams.has('page') || urlParams.has('search')) {
                const element = document.getElementById('withdrawals-history-wrapper');
                if (element) {
            ...
            $(document).on('click', '.ajax-pagination a', function(e) {
                ...
                // Show spinner
                $('#withdrawals-loading-spinner').removeClass('hidden');
                ...
                $.get(url, function(data) {
                    // Extract content
                    var newContent = $(data).find('#withdrawals-table-content').html();

                    // Update DOM
                    $('#withdrawals-table-content').html(newContent);
                }).always(function() {
                    // Hide spinner
                    setTimeout(function() {
                        $('#withdrawals-loading-spinner').addClass('hidden');
                    }, 300);
                });
            });
