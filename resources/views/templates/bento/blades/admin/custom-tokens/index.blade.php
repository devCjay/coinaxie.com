...
@section('content')
    ...
    <div class="space-y-8">
        ...
        <div class="bg-secondary border border-white/5 rounded-[2rem] overflow-hidden shadow-2xl relative">
            <div class="bg-secondary/30 border-b border-white/5 p-4 lg:p-6 relative">
                <div class="flex flex-col lg:flex-row lg:items-center justify-between gap-6">
                    ...
                    <div class="flex-1 max-w-2xl flex flex-wrap gap-3 lg:justify-end">
                        <form id="token-filter-form" method="GET" action="{{ route('admin.custom-tokens.index') }}" class="flex flex-wrap gap-3 flex-1 min-w-[260px]">
                            <div class="relative flex-1 group min-w-[200px]">
                                <input id="token-search" type="text" name="search" value="{{ request('search') }}"
                            ...
                            <select id="token-market" name="market"
            ...
            <div id="token-results">
                @include('templates.bento.blades.admin.custom-tokens.inner', ['tokens' => $tokens])
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const market = document.getElementById('token-market');
            const results = document.getElementById('token-results');
            if (!form || !search || !market) return;

            let timer = null;
            let inflight = null;
            let lastUrl = null;

            const buildUrl = function(overrides) {
            ...
            document.addEventListener('click', function(e) {
                const a = e.target && e.target.closest ? e.target.closest('#token-results a') : null;
                if (!a) return;
                const href = a.getAttribute('href');
                if (!href) return;
                const url = new URL(href, window.location.origin);
                if (url.pathname !== new URL(form.action, window.location.origin).pathname) return;
                e.preventDefault();
